...
import { addMonths, setDate, startOfMonth, endOfMonth, isBefore, isAfter, format } from 'date-fns';
import * as schema from './schema';
import { DEFAULT_CATEGORIES } from '../lib/constants';
import { newId } from '../lib/ids';

const DB_PATH = process.env.DB_FILE_NAME ?? path.join(process.cwd(), 'data', 'ample.db');
...
async function main() {
  ...
  const addTx = (
    t: Partial<TxSeed> & { accountId: string; amount: number; date: Date },
    tags?: string[],
  ) => {
    const id = newId();
    txs.push({
  ...
  for (let m = 0; m <= MONTHS_BACK; m++) {
    ...
    addTx(
      {
        ...
        amount: cents(rnd(4750, 4900)),
        date: day(1),
        payee: 'Northwind Studios',
        type: 'income',
      },
      ['essential'],
    );
      ...
      addTx(
        {
          ...
          amount: cents(rnd(300, 1200)),
          date: day(randInt(8, 20)),
    ...
    addTx(
      {
        ...
        amount: -cents(rnd(95, 175)),
        date: day(9),
        payee: pick(merchants.Utilities),
      },
      ['essential'],
    );
    addTx(
      {
        accountId: acc('checking'),
        categoryId: cat('Utilities'),
        amount: -cents(rnd(45, 70)),
        date: day(9),
        payee: 'FiberNet Internet',
      },
      ['essential'],
    );
      ...
      addTx(
        {
          ...
          amount: -cents(rnd(28, 145)),
          date: day(randInt(2, 27)),
          payee: pick(merchants.Groceries),
        },
        chance(0.3) ? ['essential'] : undefined,
      );
    }
    // dining
    for (let i = 0; i < randInt(5, 11); i++) {
      addTx({
        accountId: chance(0.85) ? acc('credit') : acc('cash'),
        categoryId: cat('Dining'),
        amount: -cents(rnd(9, 78)),
        date: day(randInt(1, 28)),
        payee: pick(merchants.Dining),
      });
    }
    // transport & fuel
    for (let i = 0; i < randInt(2, 5); i++)
      addTx({
        ...
        amount: -cents(rnd(6, 34)),
        date: day(randInt(1, 28)),
        payee: pick(merchants.Transport),
      });
    if (chance(0.7))
      addTx({
        accountId: acc('credit'),
        categoryId: cat('Fuel'),
        amount: -cents(rnd(38, 72)),
        date: day(randInt(5, 25)),
        payee: pick(merchants.Fuel),
      });
    // shopping
      ...
      addTx(
        {
          ...
          amount: -cents(rnd(18, 240)),
          date: day(randInt(1, 28)),
          payee: pick(merchants.Shopping),
        },
        chance(0.25) ? ['wishlist'] : undefined,
      );
    // occasional buckets
    if (chance(0.6))
      addTx({
        ...
        amount: -cents(rnd(12, 90)),
        date: day(randInt(1, 28)),
        payee: pick(merchants.Entertainment),
      });
    if (chance(0.4))
      addTx({
        accountId: acc('credit'),
        categoryId: cat('Health'),
        amount: -cents(rnd(15, 180)),
        date: day(randInt(1, 28)),
        payee: pick(merchants.Health),
      });
    if (chance(0.35))
      addTx({
        accountId: acc('credit'),
        categoryId: cat('Personal Care'),
        amount: -cents(rnd(25, 85)),
        date: day(randInt(1, 28)),
        payee: pick(merchants['Personal Care']),
      });
    if (chance(0.25))
      addTx(
        {
          ...
          amount: -cents(rnd(15, 60)),
          date: day(randInt(1, 28)),
          payee: pick(merchants.Education),
        },
        ['tax-deductible'],
      );
    if (chance(0.2))
      addTx({
        accountId: acc('credit'),
        categoryId: cat('Pets'),
        amount: -cents(rnd(20, 120)),
        date: day(randInt(1, 28)),
        payee: pick(merchants.Pets),
      });
    if (chance(0.18))
      addTx(
        {
          ...
          amount: -cents(rnd(120, 780)),
          date: day(randInt(1, 28)),
          payee: pick(merchants.Travel),
        },
        ['one-off'],
      );
    ...
    if (chance(0.5))
      addTransfer(acc('checking'), acc('emergency'), cents(250), day(20), 'Emergency Top-up');
