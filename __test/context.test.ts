...
async function main() {
  ...
  assert(
    'includes recurring rules + subscription total',
    /Recurring rules/.test(ctx) && /Netflix/.test(ctx),
  );
  assert('includes monthly trend', /Monthly trend/.test(ctx));
  assert('includes Debts & loans section', /Debts & loans/.test(ctx));
  assert(
    'includes the debt counterparty + reason',
    /Banco Atlántida/.test(ctx) && /Préstamo carro/.test(ctx),
  );
