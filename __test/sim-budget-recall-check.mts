...
async function seed() {
  ...
  await db.insert(transactions).values([
    ...
    { id: "t_dc", accountId: "acc_other", categoryId: "cat_dc", type: "expense", amount: -150000, currency: "HNL", date: noon("2026-08-10"), status: "cleared" },
    // over-budget: more already spent than the budget
    { id: "t_over", accountId: "acc_main", categoryId: "cat_over", type: "expense", amount: -600000, currency: "HNL", date: noon("2026-10-05"), status: "cleared" },
    // spend in an UNbudgeted category — must never be projected
    { id: "t_unb", accountId: "acc_main", categoryId: "cat_unb", type: "expense", amount: -50000, currency: "HNL", date: noon("2026-11-05"), status: "cleared" },
  ]);
}
...
async function testBudget() {
  ...
  assert("no history → default asset account (acc_main)", s.accounts.has("acc_main") && s.accounts.size === 1, [...s.accounts]);
  const label = (await db.select().from(transactions).where(eq(transactions.categoryId, "cat_food")).get())?.payee;
  ...
  const totalDc = (await db.select().from(transactions).where(and(eq(transactions.categoryId, "cat_dc"), eq(transactions.type, "expense"))))
    .filter((r) => monthKey(r.date) === "2026-08")
    .reduce((a, r) => a + -r.amount, 0);
  assert("cat_dc total Aug spend reaches budget (800000)", totalDc === 800000, totalDc);
  assert("gap booked to the category's usual account (acc_other)", s.accounts.has("acc_other") && s.accounts.size === 1, [...s.accounts]);
  ...
  const unb = await db
    .select()
    .from(transactions)
    .where(and(eq(transactions.categoryId, "cat_unb"), eq(transactions.externalId, SIM_BUDGET_MARKER)));
...
async function testRecall() {
  ...
  await appendMessage(tCar, { role: "user", content: "Quiero un plan de ahorro para un carro nuevo" });
  await appendMessage(tCar, { role: "assistant", content: "Te propongo apartar L 5000 al mes para el carro." });
  ...
  assert("index includes the car-savings thread title", recall.includes("plan de ahorro para un carro"), recall);
  ...
  assert("no 'relevant' excerpt block when nothing matches", !/relevant past conversation/i.test(recall2), recall2);
