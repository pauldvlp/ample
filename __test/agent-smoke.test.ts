...
async function main() {
  ...
  assert(
    'ctx now knows Mascotas',
    ctx.categories.some((c) => c.name === 'Mascotas'),
  );
  ...
  assert(
    'share a transfer group',
    !!transfers[0]?.transferGroupId &&
      transfers[0].transferGroupId === transfers[1]?.transferGroupId,
  );
  ...
  assert(
    'ctx knows debt Juan',
    ctx.debts.some((d) => d.counterparty === 'Juan'),
  );
  ...
  r = await executeAction({ tool: 'add_debt_payment', counterparty: 'Juan', amount: 500 }, ctx);
  ...
  console.log('\n[6] goal + contribution');
  r = await executeAction({ tool: 'create_goal', name: 'Vacaciones', targetAmount: 15000 }, ctx);
  assert('goal ok', r.ok, r);
  r = await executeAction({ tool: 'contribute_to_goal', goal: 'Vacaciones', amount: 1000 }, ctx);
  ...
  r = await executeAction({ tool: 'set_budget', category: 'Comida', amount: 4000 }, ctx);
  ...
  const rule = await db
    .select()
    .from(recurringRules)
    .where(eq(recurringRules.name, 'Netflix'))
    .get();
  ...
  assert(
    'omitted account booked to first account (Efectivo)',
  ...
  const susanaPaidBefore = await db
    .select({ s: sql<number>`COALESCE(SUM(${debtPayments.amount}),0)` })
    .from(debtPayments)
    .where(eq(debtPayments.debtId, 'debt_susana'))
    .get();
  ...
  assert('Susana debt untouched', Number(susanaPaidBefore?.s) === Number(susanaPaidAfter?.s), {
    before: susanaPaidBefore?.s,
    after: susanaPaidAfter?.s,
  });
  ...
  r = await executeAction(
    { tool: 'create_transaction', type: 'expense', amount: 10, account: 'BAC', payee: 'WordMatch' },
    ctx,
  );
  ...
  let p = normalizePlan(
    '{"message":"hi","actions":[{"tool":"create_transaction","type":"expense","amount":5}]}',
  );
  assert('standard {message,actions[]}', p.message === 'hi' && p.actions.length === 1, p);
  p = normalizePlan(
    '{"message":"ok","actions":{"tool":"create_transaction","type":"expense","amount":5}}',
  );
  assert(
    'single action object under actions -> [obj]',
    p.message === 'ok' && p.actions.length === 1,
    p,
  );
  ...
  assert(
    'pure Q&A keeps message, 0 actions',
    p.message.startsWith('just') && p.actions.length === 0,
    p,
  );
