...
async function main() {
  ...
  for (const r of routes) assert(`documents the ${r} screen`, guide.includes(r));
  ...
  assert(
    'roadmap lists the seeded planned features',
    PLANNED_FEATURES.length > 0 && PLANNED_FEATURES.every((f) => guide.includes(f.title)),
  );
