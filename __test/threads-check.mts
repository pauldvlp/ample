...
async function main() {
  ...
  assert('ordered user → assistant', msgs[0]?.role === 'user' && msgs[1]?.role === 'assistant');
  ...
  assert('updatedAt is epoch ms', typeof list[0]?.updatedAt === 'number' && list[0]!.updatedAt > 0);
  ...
  assert("deleted thread's messages cascaded", (await getThreadMessages(t1)).length === 0);
