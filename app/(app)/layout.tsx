...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  const [accountsRaw, categoriesRaw, tags, nw, series, payees, settings] = await Promise.all([
  ...
  return (
    <div className="min-h-dvh">
      ...
      <div className="lg:pl-60">
        <AppTopbar accounts={accounts} categories={categories} tags={tags} payees={payees} />
        <SimulationBanner />
        <main className="mx-auto w-full max-w-[1440px] px-4 py-6 sm:px-6 lg:px-8">{children}</main>
      </div>
      <ProductTour autoStart={!settings.tourCompleted} aiEnabled={settings.aiEnabled} />
    </div>
  );
}
