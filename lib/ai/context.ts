...
export async function buildFinanceContext(opts?: {
  ...
  const m = (c: number) => formatMoney(c, { currency: s.baseCurrency, locale: s.locale });
  const pct = (r: number) => formatPercent(r, { locale: s.locale });
  const day = (dt: Date | number) => formatDate(dt, 'yyyy-MM-dd');
  const daysFromNow = (dt: Date) => Math.round((dt.getTime() - now.getTime()) / 86_400_000);
  ...
  if (s.simulationActive) push(`NOTE: the app is in SIMULATION mode — "now" is a simulated clock.`);
  ...
  if (d.accounts.length) {
    ...
    if (d.creditUtilization.length) {
      ...
      for (const c of d.creditUtilization)
        push(`- ${c.name}: ${m(c.used)} used of ${m(c.limit)} limit (${pct(c.pct)})`);
    }
  } else {
  ...
  push(
    `Last month — income ${m(d.lastMonth.income)}, expenses ${m(
      d.lastMonth.expense,
    )}, net ${m(d.lastMonth.net)}. Spending vs last month ${pct(d.spendingDeltaPct)}.`,
  );
  if (d.incomeVsExpense.length) {
    push('Monthly trend (income / expense / net):');
    for (const mth of d.incomeVsExpense)
      push(`- ${mth.month}: ${m(mth.income)} / ${m(mth.expense)} / ${m(mth.net)}`);
  }
  ...
  if (d.budget && (d.budget.lines.length || d.budget.unbudgeted.length)) {
    ...
    for (const l of d.budget.lines) {
      ...
      push(`- ${l.categoryName}: spent ${m(l.spent)} of ${m(l.amount)} (${pct(l.pct)})${over}`);
    }
  ...
  if (recurring.length) {
    ...
    if (subs.length)
      push(`Subscriptions: ${subs.length}, ~${m(subTotal)}/mo (~${m(subTotal * 12)}/yr).`);
    ...
    for (const db of openDebts) {
      ...
      if (inst.length) {
        ...
        for (const i of pending.slice(0, 6)) {
          ...
          const when = dn < 0 ? `${-dn}d overdue` : dn === 0 ? 'today' : `in ${dn}d`;
          push(
        ...
        if (pending.length > 6) push(`      · …and ${pending.length - 6} more cuota(s)`);
      }
    }
