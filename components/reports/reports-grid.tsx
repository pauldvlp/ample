...
import type { CashFlow, CategorySpend, MonthlyFlow, NetWorthPoint } from '@/lib/data/reports';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2" data-tour="reports-charts">
        ...
        <SectionCard title={t('reports.cashFlowTitle')} description={t('reports.cashFlowDesc')}>
...
function PercentTile({ label, value, sub }: { label: string; value: number; sub?: string }) {
  return (
...
function CashFlowSummary({ flow, savingsRate }: { flow: CashFlow; savingsRate: number }) {
  const t = useT();
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-3 rounded-xl bg-muted/40 px-3 py-2.5">
        ...
        <div className="min-w-0 text-right">
          ...
          <p className="font-display text-lg font-medium tnum">{formatPercent(savingsRate)}</p>
        </div>
      </div>
    </div>
  );
