...
import { saveAiConfig, testAiConnection, fetchAiModels } from '@/lib/actions/ai-config';
import {
  AI_PROVIDER_LIST,
  PROVIDER_LABELS,
  DEFAULT_MODELS,
...
import { CheckCircle2, ShieldCheck, ExternalLink, Loader2, RefreshCw } from 'lucide-react';
...
export function AiSettings({
  ...
  const modelOptions = React.useMemo(() => {
    const base = fetchedModels && fetchedModels.length ? fetchedModels : SUGGESTED_MODELS[provider];
    const list = [...base];
  ...
  async function save(nextEnabled = enabled) {
    ...
    if (res.ok) {
      ...
      toast.error(res.error === 'missing-key' ? t('ai.missingKey') : t('ai.saveError'));
    }
  }
        ...
        <div className="space-y-4">
          ...
          <Field label={t('ai.provider')}>
            <Select
              ...
              items={Object.fromEntries(AI_PROVIDER_LIST.map((p) => [p, PROVIDER_LABELS[p]]))}
            >
            ...
            <p className="mt-1.5 text-xs text-muted-foreground">{t(`ai.tier.${provider}`)}</p>
          </Field>

          <Field label={t('ai.apiKey')} hint={keySaved ? t('ai.keySaved') : undefined}>
            <Input
              type="password"
              value={apiKey}
          ...
          <Field label={t('ai.model')} hint={t('ai.modelHint')}>
            <Select
              ...
              onValueChange={(v) => setModel(v && v !== DEFAULT_MODEL ? v : '')}
              items={{
            ...
            <p className="mt-1.5 text-xs text-muted-foreground">{t('ai.modelSafeHint')}</p>
            <button
              type="button"
              onClick={loadModels}
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button type="button" variant="outline" onClick={test} disabled={testing || !canUse}>
              {testing ? (
                <Loader2 className="size-4 animate-spin" />
              ) : (
