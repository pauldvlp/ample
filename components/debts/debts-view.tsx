    ...
    >
      {debts.length ? (
        ...
        <EmptyState
          ...
          description={
            kind === "receivable"
              ? t("debts.emptyReceivableDesc")
              : t("debts.emptyPayableDesc")
          }
        />
      )}
    </SectionCard>
...
function DebtRow({
  ...
  const hasOriginal =
    ...
    !!debt.originalCurrency &&
    debt.originalCurrency !== base;
  ...
  async function toggleSettled() {
    ...
    if (res.ok)
      toast.success(settled ? t("debts.reopenedToast") : t("debts.settledToast"));
    else toast.error(res.error);
  }
