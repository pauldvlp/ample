...
import { Field, AmountInput, ColorPicker, IconPicker } from '@/components/shared/form-fields';
...
export function GoalForm({
  ...
  const [accountId, setAccountId] = React.useState<string>(goal?.accountId ?? NO_ACCOUNT);
  const [icon, setIcon] = React.useState<string>(goal?.icon ?? 'Rocket');
  const [color, setColor] = React.useState<string>(goal?.color ?? CATEGORY_PALETTE[0]);
  ...
  async function submit(e: React.FormEvent) {
    ...
    setPending(true);
    const res = isEdit ? await updateGoal(goal!.id, input) : await createGoal(input);
    setPending(false);
    ...
    <form onSubmit={submit} className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Field label={t('goals.targetAmount')} htmlFor="goal-target">
          <AmountInput id="goal-target" value={targetAmount} onChange={setTargetAmount} />
        </Field>
        <Field label={t('goals.alreadySaved')} htmlFor="goal-current">
          <AmountInput id="goal-current" value={currentAmount} onChange={setCurrentAmount} />
        </Field>
      </div>

      <div className="grid gap-4 sm:grid-cols-2">
        <Field label={t('goals.targetDate')} htmlFor="goal-date" hint={t('common.optional')}>
          <DatePicker
            id="goal-date"
            value={targetDate}
