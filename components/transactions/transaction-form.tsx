...
export function TransactionForm({
  ...
  const [currency, setCurrency] = React.useState(
    transaction?.originalCurrency ?? base
  );
  const [amount, setAmount] = React.useState(
    transaction
      ? String(
          Math.abs(
            fromCents(transaction.originalAmount ?? transaction.amount)
          )
        )
      : ""
  );
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Field label={type === "transfer" ? t("transactions.fromAccountLabel") : t("common.account")}>
      ...
      <Field label={type === "transfer" ? t("transactions.descriptionLabel") : t("transactions.payee")}>
        {type === "transfer" ? (
          <Input
            value={payee}
