...
import { MoreHorizontal, Plus, Send, Pencil, Trash2, Repeat, Undo2 } from 'lucide-react';

function dueLabel(days: number, t: TFunction): { text: string; tone: string } {
  if (days < 0) return { text: t('recurring.overdue', { n: -days }), tone: 'text-negative' };
  ...
  if (days <= 7) return { text: t('recurring.dueInDays', { n: days }), tone: 'text-brass' };
  return {
...
function UpcomingRow({ bill }: { bill: UpcomingBill }) {
  ...
  const hasOriginal =
    bill.originalAmount != null && !!bill.originalCurrency && bill.originalCurrency !== base;
  ...
  return (
    <li className="flex items-center gap-3 py-2.5 first:pt-0">
      <IconDisc icon={bill.categoryIcon ?? 'Repeat'} color={bill.categoryColor} size="sm" />
...
function RuleRow({
  ...
  const hasOriginal =
    rule.originalAmount != null && !!rule.originalCurrency && rule.originalCurrency !== base;
  ...
  const signedMonthly = rule.type === 'income' ? rule.monthlyEquivalent : -rule.monthlyEquivalent;
  ...
  async function revertLast() {
    ...
    if (res.ok) toast.success(t('recurring.revertedToast'));
      ...
      toast.error(res.error === 'nothing-to-revert' ? t('recurring.nothingToRevert') : res.error);
  }
  ...
  return (
    ...
    >
      ...
      >
        <IconDisc icon={rule.categoryIcon ?? 'Repeat'} color={rule.categoryColor} />
        <div className="min-w-0 flex-1">
        ...
        <div className="shrink-0 text-right">
          <Amount value={signedMonthly} colored showSign className="text-sm font-medium" />
      ...
      <DropdownMenu>
        <DropdownMenuTrigger
          render={
            <Button variant="ghost" size="icon-sm" aria-label={t('recurring.moreActions')}>
        ...
        <DropdownMenuContent align="end" className="min-w-44">
          ...
          <DropdownMenuSeparator />
          <DropdownMenuItem variant="destructive" onClick={() => deleteRef.current?.click()}>
