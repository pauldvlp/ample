...
import { createCategory, updateCategory, type CategoryInput } from '@/lib/actions/categories';
...
const KIND_META: Record<CategoryKind, { labelKey: string; icon: React.ElementType; tone: string }> =
  {
    income: {
...
export function CategoryForm({
  ...
  const [kind, setKind] = React.useState<CategoryKind>(category?.kind ?? defaultKind);
  const [parentId, setParentId] = React.useState<string>(category?.parentId ?? '');
  const [icon, setIcon] = React.useState<string | null>(category?.icon ?? 'Tag');
  const [color, setColor] = React.useState<string | null>(category?.color ?? CATEGORY_PALETTE[0]);
    ...
    <form onSubmit={submit} className="space-y-4">
      ...
      <div className="flex items-center gap-3 rounded-xl bg-muted/40 px-3 py-2.5">
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs text-muted-foreground">{t(KIND_META[kind].labelKey)}</p>
        </div>
      </div>
          ...
          >
            ...
            <SelectContent>
              <SelectItem value={NO_PARENT}>
                <span className="text-muted-foreground">{t('categories.noParent')}</span>
              </SelectItem>
              {parentOptions.map((c) => (
      ...
      <div className="flex justify-end gap-2 pt-1">
        <Button type="submit" disabled={pending}>
          {pending ? t('action.saving') : isEdit ? t('action.saveChanges') : t('categories.create')}
        </Button>
      </div>
    </form>
  );
