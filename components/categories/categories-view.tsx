...
function nest(list: CategoryWithUsage[]): { cat: CategoryWithUsage; indent: boolean }[] {
...
export function CategoriesView({ categories }: { categories: CategoryWithUsage[] }) {
      ...
      {ordered.length === 0 ? (
        ...
        <div className="-mx-1 flex flex-col gap-0.5">
          {ordered.map(({ cat, indent }) => (
            <CategoryRow key={cat.id} category={cat} indent={indent} allOptions={allOptions} />
          ))}
        </div>
      )}
    </SectionCard>
...
function CategoryRow({
  ...
  async function toggleArchive() {
    ...
    setBusy(false);
    if (res.ok)
      toast.success(c.isArchived ? t('categories.toastRestored') : t('categories.toastArchived'));
